...
_lastPos = [];
{
	...
	_icon = _display ctrlCreate [(if (true) then {"RscPictureKeepAspect"} else {"RscPicture"}), -1, _slide];
	_icon ctrlSetPosition [(_pos select 0) + 1.5 * _pW, (_pos select 1) + 0.625 * _pH, 3 * _pW, 3 * _pH];
	_icon ctrlSetText _logo;
